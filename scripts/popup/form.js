...
const config = window.DonationConfig || {};
const processDonationAPI = config.api?.processDonationEndpoint || 'https://API-ENDPOINT';

(function () {
  "use strict";

  var BRAND_PRIMARY = config.branding?.BRAND_PRIMARY || "#000000";
  var BRAND_SECONDARY = config.branding?.BRAND_SECONDARY || "#ffffffff";
  var BRAND_TERTIARY = config.branding?.BRAND_TERTIARY || "#000000";
  ...
  <style id="donation-popup-style">
    :root { --brand:${BRAND_PRIMARY}; --brand-secondary:${BRAND_SECONDARY}; --brand-tertiary:${BRAND_TERTIARY}; }
  ...
  function donationDetailsHTML(prefix) {
    // Generate amount rows HTML - get amounts from configuration
    ...
    // Generate frequency options based on configuration
    const availableFrequencies = config.frequency?.availableTypes || ['onetime', 'recurring'];
    const defaultFreq = config.frequency?.defaultType || 'onetime';
    
    let frequencyOptionsHTML = '';
    if (availableFrequencies.includes('onetime')) {
      const isSelected = defaultFreq === 'onetime' ? 'selected' : '';
      frequencyOptionsHTML += `<button type="button" class="dp-chip dp-frequency-chip ${isSelected}" data-frequency="onetime">One-Time</button>`;
    }
    if (availableFrequencies.includes('recurring')) {
      const isSelected = defaultFreq === 'recurring' ? 'selected' : '';
      frequencyOptionsHTML += `<button type="button" class="dp-chip dp-frequency-chip ${isSelected}" data-frequency="recurring">Recurring</button>`;
    }
      ...
      <div class="dp-step-content active" id="${prefix}-step1">
        <div class="dp-card">
          ...
          <div style="margin-bottom:16px;">
            ...
            <input type="hidden" id="${prefix}-frequency" value="${defaultFreq}">
          </div>
          ...
          ${(config.processingFee?.showCoverFeeOption !== false) ? `
            <div class="dp-fee-checkbox-container" style="margin-bottom:12px;">
              <label class="dp-checkbox-container">
                <input type="checkbox" id="${prefix}-cover-fee" class="dp-checkbox"${(config.processingFee?.coverFeeDefaultChecked === true) ? ' checked' : ''}>
  ...
  function wireUp(prefix) {
    var currentStep = 1;
    var totalSteps = config.ui?.totalSteps || 3;
    
    // Category configuration - from config file with fallbacks
    var categoryConfig = {
      onetime: config.categories?.onetime || [
        "ONE-TIME CATEGORY"
      ],
      recurring: config.categories?.recurring || [
        "RECURRING CATEGORY"
      ]
    };
    ...
    // Only add event listener if coverFee checkbox exists (it might be hidden based on config)
    if (coverFee) {
    ...
    function computeTotals() {
      ...
      var cover = coverFee ? coverFee.checked : false;
      
      // Always calculate fee for display purposes using configurable rates
      var fee = 0;
      if (paymentMethod === "ach") {
        var achFeePercentage = config.processingFee?.achFeePercentage || 0.008;
        var achFeeMax = config.processingFee?.achFeeMax || 5.0;
        fee = Math.min(amt * achFeePercentage, achFeeMax);
      } else {
        // card & wallet: configurable rates
        var cardFeePercentage = config.processingFee?.cardFeePercentage || 0.022;
        var cardFeeFixed = config.processingFee?.cardFeeFixed || 0.30;
        fee = amt * cardFeePercentage + cardFeeFixed;
      }
    ...
    ["input","change"].forEach(function (ev) {
      ...
      coverFee && coverFee.addEventListener(ev, updateTotals);
    ...
    submitBtn.addEventListener("click", function () {
      ...
      var payload = {
        ...
        coverFee: coverFee ? coverFee.checked : false,
      ...
      .then(function (session) {
        var key = payload.livemode ? 
          (config.api?.stripePublicKeyLive || "pk_live_") : 
          (config.api?.stripePublicKeyTest || "pk_test_");
  ...
  window.renderDonationPopup = function() {
    mountPopup();
