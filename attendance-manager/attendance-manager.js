 ...
 * Single-file loader for the Attendance Manager UI. Drop this file into a Squarespace
 * site by adding: <script src="https://mprefuge.github.io/site-assets/attendance-manager/attendance-manager.js"></script>
 *
 * What this does:
 *  - injects the Attendance Manager markup into the current page
 *  - loads (and inlines if possible) the app stylesheet
 *  - loads dependencies (ExcelJS + lookup) and then loads the app scripts
 *
 * Note: this loader keeps the app files hosted on GitHub Pages (same host as existing pages)
 * and only requires a single <script> to be added to a page.
 */
(function () {
  'use strict';

  const BASE_CDN = 'https://mprefuge.github.io/site-assets';
  const CSS_URL = `${BASE_CDN}/attendance-manager/styles/attendance-manager.css`;
  const EXCEL_CDN = 'https://cdn.jsdelivr.net/npm/exceljs@4.4.0/dist/exceljs.min.js';
  const LOOKUP_JS = `${BASE_CDN}/scripts/lookup.js`;
  const SESSION_JS = `${BASE_CDN}/attendance-manager/scripts/am-session.js`;
  const WALKTHROUGH_JS = `${BASE_CDN}/attendance-manager/scripts/am-walkthrough.js`;
  const APP_JS = `${BASE_CDN}/attendance-manager/scripts/am-script.js`;

  // The HTML markup for the app (taken from attendance-manager/index.html body).
  // Kept minimal formatting so it can be injected as a single string.
  const ATT_MARKUP = `
...
<option value="Other">Other</option></select>
    ...
    <div id="att-main-section" class="att-hidden">      
      <div class="att-tabs">
        <button class="att-tab active" data-tab="register">New Registration</button>
        <button class="att-tab" data-tab="collect">Collect Attendance</button>
      ...
      <div id="att-tab-collect" class="att-tab-content att-hidden">
        ...
        <div id="att-event-inline-editor" class="att-form-row att-hidden" style="margin-bottom:12px; gap:12px; align-items:center;">
          ...
          <!-- Location input removed from inline event editor per UX: individual event edit does not require location -->
          <div style="flex:1; display:flex; gap:8px; align-items:center; margin-top:8px;">
            <div style="flex:1; position:relative;">
              <!-- intentionally empty; location is managed in Event Configuration -->
            </div>
          </div>
          <div style="display:flex; gap:8px;">
        ...
        <div id="att-attendees-section" class="att-hidden">
          ...
          <div class="att-attendee-list">
            <div id="att-attendees-loading" class="att-attendees-loading att-hidden" aria-live="polite">
              <div class="att-loading-inner">
                <span class="att-spinner" aria-hidden="true"></span>
                <div style="margin-left:8px; font-size:14px; color:var(--muted);">Loading attendees…</div>
              </div>
            </div>
      ...
      <div id="att-tab-config" class="att-tab-content att-hidden">
        ...
        <div class="att-hint-box" role="note" data-hint-id="config-setting-up-event">
          <div class="att-hint-content">
            ...
            <div class="att-hint-body">Set the default event name, location and times — these will be attached to attendance records.</div>
          </div>
        </div>
      ...
      <div id="att-tab-register" class="att-tab-content">
      ...
      <div id="att-tab-export" class="att-tab-content att-hidden">
        ...
        <div class="att-hint-box" role="note" data-hint-id="export-exporting-data">
          <div class="att-hint-content">
            ...
            <div class="att-hint-body">Pick your export type and date range, preview results, then export CSV — filters apply before export.</div>
          </div>
        </div>
        <div class="att-form-row">
          <div class="att-form-group" style="flex:1 1 100%;">
          ...
          <div class="att-form-row" style="align-items: flex-end;">
            ...
            <div class="att-form-group" style="flex: 0;">
              <button class="att-btn att-btn-primary" id="att-export-btn">
                <span id="att-export-btn-text">📥 Export</span>
  ...
  <div id="att-edit-modal" class="att-modal-overlay att-hidden">
    <div class="att-modal">
      <div class="att-modal-header">
        ...
        <button class="att-modal-close" id="att-modal-close">×</button>
      </div>
  ...
  <div id="att-walkthrough-overlay" class="att-walkthrough-overlay">
    <div class="att-walkthrough-modal">
      <div class="att-walkthrough-header">
        ...
        <button id="att-walkthrough-close" class="att-walkthrough-close">×</button>
      </div>
      ...
      <div class="att-walkthrough-footer">
        ...
        <div class="att-walkthrough-nav">
          <button id="att-walkthrough-prev" class="att-walkthrough-btn-prev">← Back</button>
          <button id="att-walkthrough-next" class="att-walkthrough-btn-next">Next →</button>
        </div>
      </div>
    </div>
  </div>
...
`;
  ...
  function appendStyleText(cssText) {
    try {
      const s = document.createElement('style');
      s.setAttribute('data-att-style', 'attendance-manager');
      s.appendChild(document.createTextNode(cssText));
      document.head.appendChild(s);
      return true;
    } catch (e) {
      console.warn('Failed to inline CSS - will fallback to <link> fallback', e);
      return false;
    }
  }

  function appendLinkHref(href) {
  ...
  function injectMarkup() {
    // attempt to inject markup inside a container where it won't conflict
    // with existing pages. We insert at the end of document.body.
    const container = document.createElement('div');
    container.setAttribute('id', 'att-manager-root');
    container.innerHTML = ATT_MARKUP;
  ...
  // Try to fetch the CSS and inline it. If it fails for any reason, attach a <link>.
  function fetchAndAttachCSS() {
    return fetch(CSS_URL, { mode: 'cors' })
      .then(resp => {
        if (!resp.ok) throw new Error('Failed to fetch stylesheet');
        return resp.text();
      })
      .then(cssText => { appendStyleText(cssText); })
      .catch((err) => {
        console.warn('Could not fetch CSS to inline:', err);
        appendLinkHref(CSS_URL);
      });
  }

  // Run the loader in correct order.
  function runLoader() {
    // 1) inject css
    fetchAndAttachCSS()
      // 2) inject markup (we inject first so app scripts can find elements immediately once loaded)
      .then(() => {
        injectMarkup();
      })
      // 3) quick charset check like original page
      .then(() => {
        try {
          const cs = document.characterSet || document.charset || '';
          if (cs && cs.toLowerCase() !== 'utf-8') {
            console.warn('attendance-manager loader warning: page not served as UTF-8 (document.characterSet=' + cs + ')');
          }
        } catch (e) { /* ignore */ }
      })
      // 4) load ExcelJS first (used by export feature)
      .then(() => loadScript(EXCEL_CDN))
      // 5) load lookup.js (populates select lists)
      .then(() => loadScript(LOOKUP_JS))
      // 6) load utility session script
      .then(() => loadScript(SESSION_JS))
      // 7) load walkthrough module (the main app loads it lazily but we include it so Tour works)
      .then(() => loadScript(WALKTHROUGH_JS))
      // 8) finally load the main app script
      .then(() => loadScript(APP_JS))
      .catch(err => {
        // If any step fails, still try to load the main app script as a last resort
        console.error('attendance-manager loader error', err);
        try { loadScript(APP_JS); } catch (e) { /* swallow */ }
      });
  }

  // If DOM is ready, run loader; otherwise wait
